...
const SearchCars = () => {
  ...
  const getData = async (dataSearch, check) => {
    ...
    try {
      ...
      const { data } = await axiosApi(
        `/car/list/?auction=${auction?.join(",")}&brand=${
          dataSearch?.brand || ""
        }&model=${dataSearch?.model || ""}&min_year=${
          dataSearch?.min_year || ""
        }&max_year=${dataSearch?.max_year || ""}&priceFrom=${
          dataSearch?.priceFrom || ""
        }&priceTo=${dataSearch?.priceTo || ""}${
          check && `&page=${check}`
        }&mileage_from=${mileageFrom}&mileage_to=${mileageTo}&color_f=${
          dataSearch.color
        }&fuel_type=${dataSearch.fuel_type}&transmission=${
          dataSearch.transmission
        }&location=${dataSearch.location}&sort_date=${
  ...
  return (
    <div className="container cars_block">
      <div
        className={`filter_menu${active ? " filter_menu_active" : ""}`}
        ...
        {loader ? (
          ...
          <>
            <div className="search_cars">
              {page.lastPage === 0 && (
                <h2 className="cars_not_found">Cars not found</h2>
              )}
              {cars?.map((car) => (
